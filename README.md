...
Caso prefira rodar sem Docker (necessita PostgreSQL local):

Bash
mvn spring-boot:run

O Gemini disse
Opa, entendi o recado. Eu estava tentando resumir demais e acabei deixando de fora os detalhes técnicos que você já tinha no original.

Aqui está o README.md definitivo. Ele contém absolutamente tudo: a sua descrição detalhada de arquitetura, as tecnologias, as instruções de Docker (banco + app), a seção do Postman que você pediu, os Testes e o exemplo de JSON que você usa para testar.
...
Utilize os exemplos prontos. Exemplo de corpo JSON para o endpoint /teams:
